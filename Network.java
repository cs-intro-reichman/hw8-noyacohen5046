...
public class Network {
     ...
     *  to allow testing the toString and getUser methods, before i-mplementing other methods. */
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    int maxMutuals = -1;
    for (int i = 0; i < userCount; i++) {
        User resultName = users[i];
            if (resultName.getName().equals(name)) {
            continue;
             }
        int mutuals = myUser.countMutual(resultName);
            if (mutuals > maxMutuals) {
                 maxMutuals = mutuals;
                 mostRecommended = resultName;
              }
     }
    ...
    for (int i = 0; i < userCount; i++) {
        User u = users[i];
        int currentCount = followeeCount(u.getName());
        if (currentCount > maxFollowers) {
            maxFollowers = currentCount;
            popularName = u.getName();
        }
    }
